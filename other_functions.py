...
def command_cd(command):
    ...
    if os.path.exists(os.path.join('.', command[3:].strip())):
        ...
        print("or you don't have the proper permssions.")
    return
...
def command_rm_all(command, cmd_not_found):
...
def command_help_parse(command, doc__):
    "print stuff for the help command"
    if command == 'help':
        command_help(doc__)
    elif command == 'help()':
        help()

def command_tarb(path):
    "execute tarb"
    tarb.tarb(path)
def command_dir():
    "print fake stuff"
...
#     return False, False
>>>>>>> c55e8a328281fab43191abc9812a8f5cd49014eb
